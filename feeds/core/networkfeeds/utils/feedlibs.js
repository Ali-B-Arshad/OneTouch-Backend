...
class FeedsLibs {
            ...
            if (!Boolean(pageId)) {
                ...
                return this.getSocialAccount([1, 2, 3], accountId, userId, teamId)
                    .then((socialAccountDetails) => {
                        ...
                        console.log(socialAccountDetails.social_id)
    ...
    getRecentFbFeeds(userId, accountId, teamId, pageId,socialAccountId) {
        return new Promise((resolve, reject) => {
            if (!Boolean(pageId)) {
                reject(new Error("Please validate the page id!"));
            } else {
                            ...
                            return this.getSocialAccount([1, 2, 3], accountId, userId, teamId)
                                    ...
                                    return accountUpdateTable.findOne({
                                        ...
                                        .then((updatedAccountData) => {
                                            ...
                                            //     console.log(socialAccountInfo)
                                                ...
                                                // updatedAccountData.updated_date,
                                                console.log(socialAccountInfo.account_type)
                                                if(socialAccountInfo.account_type==2){
                                                return this.facebookHelper.getRecentFacebookFeeds(socialAccountInfo.access_token, socialAccountInfo.social_id,"", config.get('facebook_api.app_id'), config.get('facebook_api.version'));
                                                }
                                                else{
                                                    return this.facebookHelper.getRecentFacebookFeeds(socialAccountInfo.access_token, socialAccountInfo.social_id,",type", config.get('facebook_api.app_id'), config.get('facebook_api.version'));
                                                }
                                                // }
                                            // else
                                ...
                                .then((postDetails) => {
                                    if (postDetails && postDetails.feeds && postDetails.feeds.length > 0) {
                                        ...
                                        var facebookMongoPostModelObject = new FacebookMongoPostModel();
                                        facebookMongoPostModelObject.deleteManyPosts(socialAccountId);
