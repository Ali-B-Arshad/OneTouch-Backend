...
class App {
  constructor() {
    ...
    return dbConnect.initialize()
      ...
      .then(() => {
        let port = config.get('feed_OneTouch.port');
        app.listen(port, () => {
          console.log(`service listening on ${config.get('feed_OneTouch.host_url')} with ${process.env.NODE_ENV} Environment!`);
          logger.info(`service listening on ${config.get('feed_OneTouch.host_url')} with ${process.env.NODE_ENV} Environment!`);
