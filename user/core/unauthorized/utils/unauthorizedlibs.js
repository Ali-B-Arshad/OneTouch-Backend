...
class UnAuthorizedUtils extends UserLibs {
    ...
    async createUser(info) {
        try {
            ...
            var activate2step = 0;
            if(info.activations.signupType==0){
                activate2step=2;
            }
                ...
                return userDetails.create({
                    ...
                    .then((createdUser) => {
                        ...
                        return userRewardsModel.create({
                            ...
                            is_OneTouch_ads_enabled: info.rewards.isAdsEnabled,
                    ...
                    .then(() => {
                        return userActivation.create({
                            ...
                            activate_2step_verification: activate2step,
    ...
    async sendActivationMail(userInfo) {
        try {
            if (!userInfo) {
                return { error: true, message: 'Invalid UserInfo' };
            }else{
                const planDetails = await this.getPlanDetails(userInfo.Activations.user_plan);
                if (!planDetails) {
                            return { error: true, message: 'Invalid plan' };
                        }
                else {
                            var activationLink = `${config.get('user_OneTouch.host_url')}/user/verifyEmail?email=${userInfo.email}&activationToken=${userInfo.Activations.email_validate_token}`;
                        ...
                        if (Number(userPhoneNumber) != 0) {
                            ...
                            return user.Activations.update({
                                ...
                                .then((result) => {
                                    ...
                                    resolve(otpCondition == 1 ? "OTP sent successfully, Please verify it" : "OTP sent successfully, Please verify it");
                                })
    ...
    twoStepLoginValidate(email, emailOtp) {
        return new Promise((resolve, reject) => {
            if (!email) {
                reject(new Error('Invalid Inputs'));
            } else {
                return this.getUserDetailsByEmail(email)
                    .then((user) => {
                        ...
                        if (user.Activations.activate_2step_verification == 1) {
                            if (!email )
                                throw new Error("You need to enter OTP");
                        }
                            ...
                            if (!email || !emailOtp)
                                ...
                                if (moment(user.Activations.otp_token_expire).add(10, "minutes") < moment().subtract(5, "hours").subtract(30, "minutes"))
                                    ...
                                    if (mailOtp == Number(emailOtp) ) {
                                        ...
                                        throw new Error("OTP did not match, please try again");
                                }
                            }
                        }
                    })
    ...
    sendForgotPasswordMail(userInfo) {
        return new Promise((resolve, reject) => {
            if (!userInfo) {
                ...
                var activationLink = `${config.get('user_OneTouch.mail_url')}/user/verifyPasswordToken?email=${userInfo.email}&activationToken=${userInfo.Activations.forgot_password_validate_token}`;
    ...
    verifyPasswordToken(email, activationToken) {
        ...
        return new Promise((resolve, reject) => {
            if (!email || !activationToken) {
                ...
                // Checking user existence with email
                return userDetails.findOne({
                    where: {
                        email: email
                    },
                                ...
                                return userActivation.update({
                                    ...
                                    .then((userInfo) => {
                                        if (process.env.NODE_ENV === 'production' || process.env.NODE_ENV === 'development' || process.env.NODE_ENV === 'apidevelopment') {
                                            ...
                                            var activationLink = `${config.get('user_OneTouch.mail_url')}/v1/verifyPasswordToken?email=${userInfo.email}&activationToken=${userInfo.Activations.forgot_password_validate_token}`;
                                            ...
                                            .then((userInfo) => {
                                                if (userInfo) {
                                                    if (process.env.NODE_ENV === 'production' || process.env.NODE_ENV === 'development' || process.env.NODE_ENV === 'apidevelopment') {
                                                        ...
                                                        var activationLink = `${config.get('user_OneTouch.mail_url')}/v1/verifyEmail?email=${userInfo.email}&activationToken=${userInfo.Activations.email_validate_token}`;
