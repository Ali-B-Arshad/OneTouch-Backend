        ...
        return authorizedLibs.getUserAccessToken(req.body.userScopeId)
            .then(function (userInformation) {
                if (!userInformation) {
                    ...
                    res.status(200).json({ code: 200, status: "success", user: userInformation.user, accessToken: userInformation.accessToken, maximumProfileCount: userInformation.user.userPlanDetails.account_count });
                }
            })
            .catch((error) => {
                analyticsServices.registerEvents({
