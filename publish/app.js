...
class App {
  constructor() {
    ...
    return dbConnect.initialize()
      ...
      .then(() => {
        let port = config.get('publisher_OneTouch.port');
        app.listen(port, () => {
          logger.info(`service listening on ${config.get('publisher_OneTouch.host_url')} with ${process.env.NODE_ENV} Environment!`);
          console.log(`service listening on ${config.get('publisher_OneTouch.host_url')} with ${process.env.NODE_ENV} Environment!`);
        });
      })
      .catch((error) => {
