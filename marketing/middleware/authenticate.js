        ...
        if (decodedToken) {
            ...
            if (remindingDays < 0) {
                ...
                if (redirectValueFromRequest) {
                    ...
                    res.redirect(`${config.get('user_OneTouch.host_url')}/v1/user/changePlan/?userId=${parsedToken.user_id}&currentPlan=${parsedToken.Activations.user_plan}&newPlan=0&redirectToken=${redirectToken}`);
                }
            }
            else {
                next();
