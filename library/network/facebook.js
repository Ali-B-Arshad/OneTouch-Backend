...
function getFacebookFeeds(socialId, accessToken, callback) {
    ...
    facebook.api(`${socialId}/feed?fields=link,privacy,message,source,attachments,created_time,description,reactions.limit(0).summary(true),type,application&limits=50`, (response) => {
...
function getRecentFacebookFeeds(socialId, accessToken,type,  callback) {
    //since,
    ...
    //likes.summary(true),comments.summary(true),
    //&since=${from}&untill=${currentUnixTime}&limit=50
    facebook.api(`${socialId}/feed?fields=privacy,message,attachments,created_time,application,reactions.limit(0).summary(true)${type}`, (response) => {
        if (!response || response.error) {
            ...
            getFacebookFeeds(socialId, accessToken, (error, feedDetails) => {
                ...
                if (feedDetails) {
                    feedDetails.forEach(function (feed) {
                        ...
                        if (feed.type == 'photo' || feed.type == 'album' || feed.type == 'cover_photo' || feed.type == 'status' || feed.type == 'profile_media' ) {
                                ...
                                feed.attachments.data.forEach(data => {
                                    if (data.subattachments) {
                                        data.subattachments.data.forEach(data => {
                                            if (data.type == 'photo' || feed.type == 'cover_photo' || feed.type == 'status' || feed.type == 'profile_media') {
 ...
 */
Facebook.prototype.getRecentFacebookFeeds = function (accessToken, socialId, type, facebookAppId, version) {
    return new Promise((resolve, reject) => {
        // Checking the input data, that every field is having data or not
        //since,
        //|| !since
        if (!accessToken || !socialId  || !facebookAppId || !version) {
            ...
            getRecentFacebookFeeds(socialId, accessToken, type, (error, feedDetails) => {
                // Checking whether it sent error in callback or not
                if (error != null || error) {
                    reject(error);
                }
                ...
                feedDetails.forEach(function (feed) {
                    ...
                    var feedMessage = feed.message ? feed.message : "";
                    var feedLink = feed.link ? feed.link : `https://www.facebook.com/${feedId}`;
                    var attachments = feed.attachments;
                    var reactions = feed.reactions;
                    var mediaUrls = [];
                    var isApplicationPost = false;
                    if (feed.application) {
                        var applicationId = feed.application.id;
                    ...
                    if (feed.type == 'photo' || feed.type == 'album' || feed.type == 'status' ) {
                        ...
                        if (feed.attachments && feed.attachments.data.length > 0) {
                            feed.attachments.data.forEach(data => {
                                if (data.subattachments) {
                                    ...
                                    if (data.type == 'photo' || data.type == 'profile_media' || data.type == 'map' || data.type == 'cover_photo') {
                                        var media = data.media.image.src;
                                        mediaUrls.push(media);
                                    }
                                }
