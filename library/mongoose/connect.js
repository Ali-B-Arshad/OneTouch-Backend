...
MongoConnect.prototype.initialize = function () {
    return new Promise((resolve, reject) => {
        if (this.mongoConfiguration.username != '' && this.mongoConfiguration.password != '') {
            mongoose.connect(`mongodb+srv://${this.mongoConfiguration.username}:${this.mongoConfiguration.password}@${this.mongoConfiguration.host}/${this.mongoConfiguration.db_name}`, { useNewUrlParser: true, useFindAndModify: false, useUnifiedTopology: true });
        } else {
